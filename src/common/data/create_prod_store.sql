CREATE DATABASE  IF NOT EXISTS `prod_store`;
USE `prod_store`;
...
CREATE TABLE `code` (
  ...
  `depth` int DEFAULT NULL COMMENT '0 : 코드 종류, 1 대분류 : 2 중분류 3: 소분류 ...',
  `name` varchar(255) NOT NULL,
  `description` varchar(255) NOT NULL,
  `position` int NOT NULL DEFAULT '1' COMMENT '순서',
  `is_disabled` tinyint(1) DEFAULT '0' COMMENT '비활성 여부',
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `coupon` (
  ...
  `is_duplicate` tinyint(1) DEFAULT '0' COMMENT '중복 적용 가능 여부',
  `benefit` varchar(255) DEFAULT NULL COMMENT '적용구분(할인: discount,적립: accumulate)',
  `min_price` int DEFAULT NULL COMMENT '최소 이용 금액',
  ...
  `limit_count` int DEFAULT NULL COMMENT '인당 제한 수',
  `download_count` int DEFAULT NULL COMMENT '다운로드 수',
  `is_disabled` tinyint(1) DEFAULT '0' COMMENT '비활성 여부',
  `store_id` int DEFAULT NULL COMMENT '가게ID',
  `created_by` int DEFAULT NULL COMMENT '등록인',
  `updated_by` int DEFAULT NULL COMMENT '수정인',
  PRIMARY KEY (`id`)
);

-- Table structure for table `coupon_disabled_history`
DROP TABLE IF EXISTS `coupon_disabled_history`;
...
-- Table structure for table `coupon_download`
DROP TABLE IF EXISTS `coupon_download`;
CREATE TABLE `coupon_download` (
  ...
  `coupon_id` int NOT NULL COMMENT '쿠폰 아이디',
  `order_id` int DEFAULT NULL COMMENT '주문 아이디',
...
-- Table structure for table `discount_category_menu`
DROP TABLE IF EXISTS `discount_category_menu`;
...
CREATE TABLE `event` (
  ...
  `updated_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  `deleted_at` timestamp(6) NULL DEFAULT NULL,
  `owner_id` int NOT NULL COMMENT '관리자(작성자) 아이디',
  ...
  `is_displayed` tinyint(1) DEFAULT '1' COMMENT '노출 여부',
  `banner_url` varchar(255) DEFAULT NULL COMMENT '배너 이미지 URL',
  `image_url` varchar(255) DEFAULT NULL COMMENT '첨부파일 이미지 URL',
  `hit_count` int DEFAULT '0' COMMENT '조회수',
  PRIMARY KEY (`id`)
);

-- Table structure for table `iamport_certification`
DROP TABLE IF EXISTS `iamport_certification`;
CREATE TABLE `iamport_certification` (
  ...
  `unique_key` varchar(100) DEFAULT NULL COMMENT '개인 고유구분 식별키(다날 매뉴얼 기준 CI)',
  `unique_in_site` varchar(100) DEFAULT NULL COMMENT '가맹점 내 개인 고유구분 식별키(다날 매뉴얼 기준 DI). 본인인증 PG MID별로 할당되는 개인 식별키',
  `name` varchar(50) DEFAULT NULL COMMENT '인증결과-실명',
  `gender` varchar(50) DEFAULT NULL COMMENT '인증결과-성별. male:남성, female:여성',
  `birthday` varchar(50) DEFAULT NULL COMMENT '인증결과-생년월일 ISO8601 형식의 문자열. YYYY-MM-DD 10자리 문자열',
  PRIMARY KEY (`id`)
);

-- Table structure for table `iamport_payment`
DROP TABLE IF EXISTS `iamport_payment`;
CREATE TABLE `iamport_payment` (
  ...
  `pay_method` varchar(50) DEFAULT NULL COMMENT 'samsung : 삼성페이 / card : 신용카드 / trans : 계좌이체 / vbank : 가상계좌 / phone : 휴대폰 / cultureland : 문화상품권 / smartculture : 스마트문상 / booknlife : 도서문화상품권 / happymoney : 해피머니 / point : 포인트 / ssgpay : SSGPAY / lpay : LPAY / payco : 페이코 / kakaopay : 카카오페이 / tosspay : 토스 / naverpay : 네이버페이',
  `channel` varchar(10) DEFAULT NULL COMMENT '결제가 발생된 경로. pc:(인증방식)PC결제, mobile:(인증방식)모바일결제, api:정기결제 또는 비인증방식결제 = [pc, mobile, api]',
  `pg_provider` varchar(20) DEFAULT NULL COMMENT 'PG사 명칭. inicis(이니시스) / nice(나이스정보통신)',
  `emb_pg_provider` varchar(20) DEFAULT NULL COMMENT '허브형결제 PG사 명칭. chai(차이) / kakaopay(카카오페이)',
  `pg_tid` varchar(50) DEFAULT NULL COMMENT 'PG사 승인정보',
  `pg_id` varchar(50) DEFAULT NULL COMMENT '거래가 처리된 PG사 상점아이디',
  `escrow` tinyint(1) DEFAULT '0' COMMENT '에스크로결제 여부',
  `apply_num` varchar(50) DEFAULT NULL COMMENT '카드사 승인정보(계좌이체/가상계좌는 값 없음)',
  `bank_code` varchar(50) DEFAULT NULL COMMENT '은행 표준코드 - (금융결제원기준)',
  `bank_name` varchar(50) DEFAULT NULL COMMENT '은행 명칭 - (실시간계좌이체 결제 건의 경우)',
  `card_code` varchar(10) DEFAULT NULL COMMENT '카드사 코드번호(금융결제원 표준코드번호) = [361(BC카드), 364(광주카드), 365(삼성카드), 366(신한카드), 367(현대카드), 368(롯데카드), 369(수협카드), 370(씨티카드), 371(NH카드), 372(전북카드), 373(제주카드), 374(하나SK카드), 381(KB국민카드), 041(우리카드), 071(우체국)]',
  `card_name` varchar(10) DEFAULT NULL COMMENT '카드사 명칭 - (신용카드 결제 건의 경우)',
  `card_quota` int DEFAULT NULL COMMENT '할부개월 수(0이면 일시불)',
  `card_number` varchar(50) DEFAULT NULL COMMENT '결제에 사용된 마스킹된 카드번호. 7~12번째 자리를 마스킹하는 것이 일반적이지만, PG사의 정책/설정에 따라 다소 차이가 있을 수 있음',
  `card_type` int DEFAULT NULL COMMENT '카드유형. (주의)해당 정보를 제공하지 않는 일부 PG사의 경우 null 로 응답됨(ex. JTNet, 이니시스-빌링) = [null, 0(신용카드), 1(체크카드)]',
  `vbank_code` varchar(50) DEFAULT NULL COMMENT '가상계좌 은행 표준코드 - (금융결제원기준)',
  `vbank_name` varchar(50) DEFAULT NULL COMMENT '입금받을 가상계좌 은행명',
  `vbank_num` varchar(50) DEFAULT NULL COMMENT '입금받을 가상계좌 계좌번호',
  `vbank_holder` varchar(50) DEFAULT NULL COMMENT '입금받을 가상계좌 예금주',
  `vbank_date` int DEFAULT NULL COMMENT '입금받을 가상계좌 마감기한 UNIX timestamp',
  `vbank_issued_at` int DEFAULT NULL COMMENT '가상계좌 생성 시각 UNIX timestamp',
  `name` varchar(50) DEFAULT NULL COMMENT '주문명칭',
  `amount` int DEFAULT NULL COMMENT '주문(결제)금액',
  `cancel_amount` int DEFAULT NULL COMMENT '결제취소금액',
  `currency` varchar(10) DEFAULT NULL COMMENT '결제승인화폐단위(KRW:원, USD:미화달러, EUR:유로)',
  `buyer_name` varchar(50) DEFAULT NULL COMMENT '주문자명',
  `buyer_email` varchar(50) DEFAULT NULL COMMENT '주문자 Email주소',
  `buyer_tel` varchar(20) DEFAULT NULL COMMENT '주문자 전화번호',
  `buyer_addr` varchar(255) DEFAULT NULL COMMENT '주문자 주소',
  `buyer_postcode` varchar(10) DEFAULT NULL COMMENT '주문자 우편번호',
  `custom_data` varchar(255) DEFAULT NULL COMMENT '가맹점에서 전달한 custom data. JSON string으로 전달',
  `user_agent` varchar(255) DEFAULT NULL COMMENT '구매자가 결제를 시작한 단말기의 UserAgent 문자열',
  `status` varchar(20) DEFAULT NULL COMMENT '결제상태. ready:미결제, paid:결제완료, cancelled:결제취소, failed:결제실패 = [ready, paid, cancelled, failed]',
  `started_at` int DEFAULT NULL COMMENT '결제시작시점 UNIX timestamp. IMP.request_pay() 를 통해 결제창을 최초 오픈한 시각',
  `paid_at` int DEFAULT NULL COMMENT '결제완료시점 UNIX timestamp. 결제완료가 아닐 경우 0',
  `failed_at` int DEFAULT NULL COMMENT '결제실패시점 UNIX timestamp. 결제실패가 아닐 경우 0',
  `cancelled_at` int DEFAULT NULL COMMENT '결제취소시점 UNIX timestamp. 결제취소가 아닐 경우 0',
  `fail_reason` varchar(250) DEFAULT NULL COMMENT '결제실패 사유',
  `cancel_reason` varchar(250) DEFAULT NULL COMMENT '결제취소 사유',
  `receipt_url` varchar(255) DEFAULT NULL COMMENT '신용카드 매출전표 확인 URL',
  `cash_receipt_issued` tinyint(1) DEFAULT '0' COMMENT '현금영수증 자동발급 여부',
  `customer_uid` varchar(50) DEFAULT NULL COMMENT '해당 결제처리에 사용된 customer_uid. 결제창을 통해 빌링키 발급 성공한 결제건의 경우 요청된 customer_uid 값을 응답합니다.',
  `customer_uid_usage` varchar(20) DEFAULT NULL COMMENT 'customer_uid가 결제처리에 사용된 상세 용도.(null:일반결제, issue:빌링키 발급, payment:결제, payment.scheduled:예약결제 = [issue, payment, payment.scheduled]',
  PRIMARY KEY (`id`)
);

-- Table structure for table `iamport_payment_cancel`
DROP TABLE IF EXISTS `iamport_payment_cancel`;
CREATE TABLE `iamport_payment_cancel` (
  ...
  `deleted_at` timestamp(6) NULL DEFAULT NULL,
  `pg_tid` varchar(50) DEFAULT NULL COMMENT 'PG사 승인정보',
  `amount` int DEFAULT NULL COMMENT '취소 금액',
  `cancelled_at` int DEFAULT NULL COMMENT '결제취소된 시각 UNIX timestamp',
  `reason` varchar(250) DEFAULT NULL COMMENT '결제취소 사유',
  `receipt_url` varchar(255) DEFAULT NULL COMMENT '취소에 대한 매출전표 확인 URL. PG사에 따라 제공되지 않는 경우도 있음',
  PRIMARY KEY (`id`)
);
...
-- Table structure for table `icon_menu`
DROP TABLE IF EXISTS `icon_menu`;
...
-- Table structure for table `icon_store`
DROP TABLE IF EXISTS `icon_store`;
...
-- Table structure for table `menu_signature`
DROP TABLE IF EXISTS `menu_signature`;
...
CREATE TABLE `option_group` (
  ...
  `name` varchar(50) NOT NULL COMMENT '옵션그룹명, 예) 필수 옵션 or 토핑 선택',
  `is_required` tinyint(1) DEFAULT '1' COMMENT '필수 여부, 예) 옵션을 반드시 선택해야 주문이 가능해요',
  `min_option` int NOT NULL COMMENT '최소 옵션수, 필수일 경우 최소 1 선택일 경우 0',
  `max_option` int NOT NULL COMMENT '최대 옵션수, 옵션 그룹에 포함된 옵션의 갯수까지',
...
CREATE TABLE `order` (
  ...
  `order_status` varchar(50) DEFAULT 'ready' COMMENT '주문상황, 주문확인/주문취소(고객취소|매장취소)/주문접수/조리중/조리완료/픽업완료',
  ...
  `type` varchar(20) DEFAULT NULL COMMENT '주소체계(지번-jibun|도로명-road)',
...
-- Table structure for table `order_menu_price`
DROP TABLE IF EXISTS `order_menu_price`;
...
-- Table structure for table `order_option_group`
...
CREATE TABLE `owner` (
  ...
  `is_confirmed` tinyint(1) DEFAULT '0' COMMENT '가게인증 여부 - 점주일 경우 필수 항목',
  `is_benefit_noti` tinyint(1) DEFAULT '0' COMMENT '동네가게 혜택 알림 동의 - 점주일 경우 필수 항목',
  `username` varchar(255) NOT NULL,
  `password` varchar(255) NOT NULL,
  `phone_number` varchar(255) DEFAULT NULL COMMENT '휴대전화 - 점주일 경우 필수 항목',
  ...
  `department` varchar(20) DEFAULT NULL COMMENT '소속 부서 - 운영자 전용',
  `memo` varchar(2000) DEFAULT NULL COMMENT '회원 메모 - 점주 전용',
...
-- Table structure for table `payment_snapshot`
DROP TABLE IF EXISTS `payment_snapshot`;
CREATE TABLE `payment_snapshot` (
  ...
  `owner_id` int DEFAULT NULL COMMENT '등록자(관리자|운영자) 아이디',
  PRIMARY KEY (`id`)
);
...
-- Table structure for table `pos_banner`
DROP TABLE IF EXISTS `pos_banner`;
...
CREATE TABLE `push_history` (
  ...
  `updated_at` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  `deleted_at` timestamp(6) NULL DEFAULT NULL,
  `device_token` varchar(200) DEFAULT NULL COMMENT '디바이스 토큰',
  `title` varchar(100) DEFAULT NULL COMMENT '제목',
  `body` varchar(500) DEFAULT NULL COMMENT '내용',
  `image_url` varchar(255) DEFAULT NULL COMMENT '이미지 URL',
  `is_succeed` tinyint(1) DEFAULT '1' COMMENT '성공 여부',
  `failure_code` varchar(200) DEFAULT NULL COMMENT '실패 코드',
  `failure_message` varchar(500) DEFAULT NULL COMMENT '실패 메세지',
  PRIMARY KEY (`id`)
);

-- Table structure for table `push_send_queue`
DROP TABLE IF EXISTS `push_send_queue`;
CREATE TABLE `push_send_queue` (
  ...
  `status` varchar(10) DEFAULT NULL COMMENT '상태( ready:준비, failure:실패, success:성공 ) ',
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `question` (
  ...
  `category` varchar(20) DEFAULT 'default' COMMENT '문의 유형(유형-default|이용관련-usage|회원관련-member|가게관련-store|결제관련-payment|기타-etc)',
  `user_id` int DEFAULT NULL COMMENT '고객(작성자) 아이디',
  `owner_id` int DEFAULT NULL COMMENT '점주(작성자) 아이디',
  `title` varchar(100) NOT NULL COMMENT '문의 제목',
  `body` varchar(5000) NOT NULL COMMENT '문의 내용',
  `image_url1` varchar(255) DEFAULT NULL COMMENT '첨부파일 이미지1 URL',
  `image_url2` varchar(255) DEFAULT NULL COMMENT '첨부파일 이미지2 URL',
...
CREATE TABLE `question_answer` (
  ...
  `owner_id` int NOT NULL COMMENT '운영자(작성자) 아이디',
  `body` varchar(1000) DEFAULT NULL COMMENT '답변 내용',
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `recommend_category` (
  ...
  `started_at` timestamp(6) NULL DEFAULT NULL COMMENT '시작일',
  `ended_at` timestamp(6) NULL DEFAULT NULL COMMENT '종료일',
  `is_all_areas` tinyint(1) DEFAULT '0' COMMENT '노출지역 전체 여부',
  `is_displayed` tinyint(1) DEFAULT '0' COMMENT '노출 여부',
  `position` int NOT NULL,
  PRIMARY KEY (`id`)
);

-- Table structure for table `recommend_category_area`
DROP TABLE IF EXISTS `recommend_category_area`;
...
-- Table structure for table `recommend_category_menu`
DROP TABLE IF EXISTS `recommend_category_menu`;
...
-- Table structure for table `settlement_fee`
DROP TABLE IF EXISTS `settlement_fee`;
...
-- Table structure for table `settlement_sum`
DROP TABLE IF EXISTS `settlement_sum`;
...
-- Table structure for table `settlement_sum_snapshot`
DROP TABLE IF EXISTS `settlement_sum_snapshot`;
...
CREATE TABLE `store` (
  ...
  `relay_to` varchar(20) DEFAULT 'POS',
  ...
  `is_package` tinyint(1) DEFAULT '1',
  `address1` varchar(255) DEFAULT NULL,
  `address2` varchar(255) DEFAULT NULL,
  ...
  `is_break_hours` tinyint(1) DEFAULT '1',
  `is_rest_days` tinyint(1) DEFAULT '0',
...
-- Table structure for table `store_break_hour`
DROP TABLE IF EXISTS `store_break_hour`;
...
-- Table structure for table `store_open_hour`
DROP TABLE IF EXISTS `store_open_hour`;
CREATE TABLE `store_open_hour` (
  ...
  `is_all_hours` tinyint(1) DEFAULT '0' COMMENT '24시간 영업 유무',
  `is_rest_day` tinyint(1) DEFAULT '0' COMMENT '휴무일 유무',
...
CREATE TABLE `store_review` (
  ...
  `store_id` int NOT NULL,
  `order_id` int NOT NULL,
  `taste` tinyint(1) DEFAULT '0' COMMENT '맛(0 ~ 1)',
  `portion` tinyint(1) DEFAULT '0' COMMENT '양(0 ~ 1)',
  `hygiene` tinyint(1) DEFAULT '0' COMMENT '위생[포장](0 ~ 1)',
...
-- Table structure for table `store_review_image`
DROP TABLE IF EXISTS `store_review_image`;
...
-- Table structure for table `store_stop_hour`
DROP TABLE IF EXISTS `store_stop_hour`;
CREATE TABLE `store_stop_hour` (
  ...
  `deleted_at` timestamp(6) NULL DEFAULT NULL,
  `store_id` int NOT NULL,
  `is_not_settings` tinyint(1) DEFAULT '0' COMMENT '시간 설정 안함(설정안함-true, 설정함-false)',
...
-- Table structure for table `tray_option`
DROP TABLE IF EXISTS `tray_option`;
...
-- Table structure for table `tray_option_group`
DROP TABLE IF EXISTS `tray_option_group`;
...
CREATE TABLE `user` (
  ...
  `is_benefit_noti` tinyint(1) DEFAULT '0',
  `is_benefit_noti_at` timestamp(6) NULL DEFAULT NULL COMMENT '동네가게 혜택 알림 여부 일자',
  `is_event_noti` tinyint(1) DEFAULT '0' COMMENT '이벤트 알람',
  `is_event_noti_at` timestamp(6) NULL DEFAULT NULL COMMENT '이벤트 알람 여부 일자',
  `is_discount_noti` tinyint(1) DEFAULT '0' COMMENT '할인 알람',
  `is_discount_noti_at` timestamp(6) NULL DEFAULT NULL COMMENT '할인 알람',
  `is_delivery_noti` tinyint(1) DEFAULT '0' COMMENT '배달 현황 알람',
  `is_email_noti` tinyint(1) DEFAULT '0' COMMENT '이메일 알림 여부',
  `is_email_noti_at` timestamp(6) NULL DEFAULT NULL COMMENT '이메일 알림 여부 일자',
  `is_sms_noti` tinyint(1) DEFAULT '0' COMMENT 'SMS 알림 여부',
  `is_sms_noti_at` timestamp(6) NULL DEFAULT NULL COMMENT 'SMS 알림 여부 일자',
  `withdrawal_reason` varchar(255) DEFAULT NULL COMMENT '회원 탈퇴 사유',
  `level` int DEFAULT NULL COMMENT '회원레벨(일반회원_C1-1|바우처회원_C2-2|C3_기부회원-3)',
  `rank` varchar(20) DEFAULT NULL COMMENT '회원등급(새내기-rookie|주민-local|이웃-neighbor|단골-regular)',
...
CREATE TABLE `user_device` (
  ...
  `deleted_at` timestamp(6) NULL DEFAULT NULL,
  `device_token` varchar(200) DEFAULT NULL COMMENT '디바이스 토큰',
  `user_id` int DEFAULT NULL COMMENT '유저아이디',
  PRIMARY KEY (`id`)
);
